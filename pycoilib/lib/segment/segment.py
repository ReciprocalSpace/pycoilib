...
import numpy as np
from numpy import cos, sin, sqrt, pi as π
...
class Loop(ArcAbstract):
    def __init__(self, radius, pos=_vec_0, axis=_vec_z, angle=0., current=1.):
        ...
        super().__init__(radius, 2 * π, pos, vec_u, vec_v, vec_w, current)
...
class Line(Segment):
    def __init__(self, p0: np.ndarray, p1: np.ndarray, current=1.):
        ...
        self.ell = sqrt((p1 - p0) @ (p1 - p0))  # ell: length
    ...
    def rotate(self, axis: np.ndarray, angle: float, origin="self.vec_r0"):
